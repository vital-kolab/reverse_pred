...
def main(model, monkey, n_neurons):
    out_dir = f'./results_predictions/monkey2model/{monkey}/v4_sampling/'
    os.makedirs(out_dir, exist_ok=True)
    rng = np.random.default_rng(seed=n_neurons)

    # Load once
    rates, selected_rates = load_selected_rates(monkey)
    pred_path = os.path.join(out_dir, f'prediction_{model}.npy')
    ev_path = os.path.join(out_dir, f'ev_{model}_{n_neurons}.npy')
    
    ev_sampling = []
    for r in range(10):
        sampled_indices = rng.choice(rates.shape[1], size=n_neurons, replace=False)
        rates_sampled = rates[:, sampled_indices]
        
        model_features = load_model_features(model)
        ev = np.load(f'./results_predictions/monkey2model/{monkey}/v4/ev_{model}.npy')
        worst_neurons = model_features[:, ev < np.nanpercentile(ev, 20, axis=0)]
        worst_neurons = worst_neurons[:,:500]
        responses = np.mean(rates_sampled, axis=2)  # (1320, n_units)
        print(responses.shape)

        prediction = mu.get_all_preds(worst_neurons, responses, ncomp=20)
        ev = mu.get_all_stats(prediction, worst_neurons, rates_sampled, ncomp=20)
...
if __name__ == "__main__":
    #if len(sys.argv) != 5:
    #    print("Usage: python model2neurongroup_slurm.py <model> <monkey> <start_index> <group_size>")
    #    sys.exit(1)

    model = sys.argv[1]
    monkey = sys.argv[2]
    n_neurons = int(sys.argv[3])
    main(model, monkey, n_neurons)
