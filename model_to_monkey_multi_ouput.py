...
def main(model, monkey):
    ...
    responses = np.mean(selected_rates, axis=2)  # (1320, n_units)
    print(responses.shape)

    pred_path = os.path.join(out_dir, f'prediction_{monkey}.npy')
    ev_path = os.path.join(out_dir, f'ev_{monkey}.npy')

    prediction = mu.get_all_preds(responses, model_features, ncomp=20, model=model, monkey=monkey)
    ev = mu.get_all_stats(prediction, selected_rates, model_features, ncomp=20)
    np.save(ev_path, ev)
