...
def main(model, monkey):
    ...
    ev_sampling = []
    for r in range(10):
        #sampled_indices = rng.choice(model_features.shape[1], size=n_neurons, replace=False)
        #model_features_sampled = model_features[:, sampled_indices]
        ev = predict_all(model_features, worst_neurons)
