...
def main(model1, model2):
    out_dir = f'./results_predictions/model2model/{model2}/v2'
    os.makedirs(out_dir, exist_ok=True)

    # Load once
    model_features = load_model_features(model1)
    ev = np.load(f'./results_predictions/monkey2model/v2/magneto/ev_{model1}.npy')
    worst_neurons = model_features[:, ev < np.nanpercentile(ev, 20, axis=0)]

    predictor_model = load_predictor_model_features(model2)
    print(predictor_model.shape)

    pred_path = os.path.join(out_dir, f'prediction_{model1}.npy')
    ev_path = os.path.join(out_dir, f'ev_{model1}.npy')

    prediction = mu.get_all_preds(worst_neurons, predictor_model, ncomp=20)
    ev = mu.get_all_stats(prediction, worst_neurons, predictor_model, ncomp=20)
    np.save(ev_path, ev)
