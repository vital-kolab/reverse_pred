...
def main(model, monkey):
    out_dir = f'./results_predictions/monkey2model/{monkey}/v4'
    os.makedirs(out_dir, exist_ok=True)

    # Load once
    rates, selected_rates = load_selected_rates(monkey)
    model_features = load_model_features(model)
    responses = np.mean(rates, axis=2)  # (1320, n_units)
    print(responses.shape)

    pred_path = os.path.join(out_dir, f'prediction_{model}.npy')
    ev_path = os.path.join(out_dir, f'ev_{model}.npy')

    prediction = mu.get_all_preds(model_features, responses, ncomp=20)
    ev = mu.get_all_stats(prediction, model_features, rates, ncomp=20)
    np.save(ev_path, ev)
